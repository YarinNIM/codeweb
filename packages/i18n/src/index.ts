...
const translateFnc = (i18n: any, key: string, replace: Record<string, any> = {}) => {
  const translation: any = i18n[key.toLowerCase()] || key
    .split('.')
    .reduce((t, i) => t[i.toLowerCase()] || key, i18n);
...
const hasPhrases = (): boolean => !!localStorage.getItem(LOCALE_PHRASE_KEY) || false;
...
export const getLocale = (): string => {
  const locale = localStorage.getItem(LOCALE_KEY) || navigator.language || false;
